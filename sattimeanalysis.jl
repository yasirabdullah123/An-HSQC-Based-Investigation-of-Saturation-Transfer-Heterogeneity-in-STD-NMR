...
aligned_peaks = NMRAnalysisUtils.load_and_align_peaks("master-peak-list-final.tsv", "33", x_shift=0.0, y_shift=0.0)
x = aligned_peaks.x
y = aligned_peaks.y
...
II0 = map(1:length(x)) do i
    ...
    # Apply uncertainty filtering (25% threshold - very strict)
    filtered_ratios = map(ratios) do ratio
        if Measurements.uncertainty(ratio) > 0.25
            return NaN ± 0.0  # Set to NaN if uncertainty is too high
        else
            return ratio
        end
    end
