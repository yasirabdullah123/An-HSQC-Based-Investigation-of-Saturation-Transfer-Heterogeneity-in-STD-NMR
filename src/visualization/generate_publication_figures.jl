...
const config = (
    ...
    # Enhanced thresholds for ultimate analysis
...
# Custom data processing functions for the ultimate script
...
# --- 3. ULTIMATE FIGURE GENERATION ---

function generate_figure3_ultimate(all_fits)
    """Generate the ultimate multi-panel kinetic portrait (Figure 3)"""
    println("🎨 GENERATING ULTIMATE FIGURE 3: Global Kinetic Portrait")
...
function generate_figure4_ultimate(differential_data, comprehensive_data, sat_freqs)
    """
    ULTIMATE Figure 4: Diagnostic Blind Spot Analysis with 2D Density Enhancement
    ...
    println("    ✓ Ultimate Figure 12 saved: $output_path")
...
function generate_figure13_ultimate(detailed_results)
    """
    ULTIMATE Figure 13: True Split Raincloud Plot for Pulse Uniformity Analysis
    Shows complete distributions of both on-target efficiency and off-target bleeding.
    """
    println("\n🚀 GENERATING ULTIMATE FIGURE 13: True Split Raincloud Analysis")
    ...
    output_path = joinpath(config.output_dir, "Figure13_Ultimate_Split_Raincloud.svg")
    savefig(p_pulse, output_path)
    println("    ✓ Ultimate Figure 6 (Split Raincloud) saved: $output_path")
...
function generate_ultimate_pulse_summary(detailed_results, pulse_names)
    """Generate comprehensive pulse performance summary"""
    summary_path = joinpath(config.output_dir, "Figure13_Ultimate_Pulse_Summary.txt")
    
    open(summary_path, "w") do f
        println(f, "ULTIMATE PULSE PERFORMANCE ANALYSIS")
        println(f, "="^50)
    ...
    println("    ✓ Pulse summary saved: Figure6_Ultimate_Pulse_Summary.txt")
end

function generate_ultimate_summary_report(archetypes, n_blind_spots, best_pulse, all_fits, differential_data)
    """Generate the ultimate comprehensive analysis report"""
    println("\n📊 GENERATING ULTIMATE SUMMARY REPORT")
    
    report_path = joinpath(config.output_dir, "Ultimate_Analysis_Report.txt")
    
    # Calculate total observable sites dynamically
    total_observable_sites = length(differential_data.keys)
    
    open(report_path, "w") do f
        println(f, "ULTIMATE DISSERTATION ANALYSIS REPORT (v5.0)")
        println(f, "="^60)
        println(f, "PARADIGM-SHIFTING VISUALIZATION AND ANALYSIS")
        ...
        println(f, "-"^45)
        println(f, "BREAKTHROUGH: Multi-panel visualization demonstrates widespread")
        ...
        println(f, "PARADIGM SHIFT ACHIEVED:")
        println(f, "-"^25) 
        ...
        println(f, "END OF ULTIMATE ANALYSIS")
    end
    
    println("✅ Ultimate summary report saved: Ultimate_Analysis_Report.txt")
end

# --- 4. MAIN EXECUTION ---

function main()
    println("🚀 STARTING ULTIMATE DISSERTATION FIGURE GENERATION (v5.0) 🚀")
    ...
    try
        ...
        if !isempty(all_fits)
            ...
            returned_archetypes = generate_figure3_ultimate(all_fits)
        ...
        if !isempty(differential_data.keys)
            n_blind_spots = generate_figure4_ultimate(differential_data, comprehensive_data, sat_freqs)
        else
        ...
        if !isempty(detailed_results)
            # Generate Figure 6
            best_pulse = generate_figure13_ultimate(detailed_results)
            generate_ultimate_pulse_summary(detailed_results, config.pulse_labels)
        else
        ...
        generate_ultimate_summary_report(archetypes, n_blind_spots, best_pulse, all_fits, differential_data)
